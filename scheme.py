...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals!=nil or vals!=nil:
            raise SchemeError("num of formals doesn't equal to that of vals")
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    get_first = lambda pair:pair.first
    get_second = lambda pair:scheme_eval(pair.second.first, env)

    def get_all_formals(pair):
        if pair == nil:
            return nil
        return Pair(get_first(pair.first), get_all_formals(pair.second))

    def get_all_vals(pair):
        if pair == nil:
            return nil
        check_form(pair.first.second, 1, 1)
        return Pair(get_second(pair.first), get_all_vals(pair.second))

    formals = get_all_formals(bindings)
    check_formals(formals)
    return env.make_child_frame(formals, get_all_vals(bindings))
    # END PROBLEM 15
...
# e = create_global_frame()
# expr = read_line("(load 'questions)")
# scheme_eval(expr, e)
# expr = read_line("(enumerate '(3 4 5 6))")
# scheme_eval(expr, e)